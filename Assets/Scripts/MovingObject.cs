...
namespace Completed
{
	...
	{
		public float moveSpeed = 3f;			//Time it will take object to move, in seconds.
		...
		protected bool Move (float xDir, float yDir, out RaycastHit2D hit)
		{
			...
			Vector2 end = start + new Vector2 (xDir, yDir) * moveSpeed * Time.deltaTime; ;
			...
			{
                var moveVector = new Vector3(xDir, yDir, 0);
                rb2D.MovePosition(new Vector2((transform.position.x + moveVector.x * moveSpeed * Time.deltaTime),
